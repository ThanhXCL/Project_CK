...
from control.control import normalize_key
from .common import recv_with_len, send_json, recv_json
from pynput import mouse, keyboard

class RemoteClient:
    def __init__(self, host='127.0.0.1', port_cmd=8888, port_vid=8889, password='123456', frame_callback=None, server_stop_callback=None):
        self.host = host
        ...
        try:
            while not self._stop.is_set() and self.vid:
                ...
                if self.frame_callback:
                    try:
                        # Nếu frame_callback là signal (UI truyền vào)
    ...
    def _listen_keyboard(self):
        ...
        def _normalize_key(k):
            # Nếu là ký tự thường (printable), trả về char
            if hasattr(k, "char") and k.char:
                if ord(k.char) >= 32:  # bỏ qua control char
                    return k.char
                return None
            # Nếu là special thì normalize
        ...
        def on_press(key):
            ...
            if len(norm) == 1 and norm.isprintable():
                    # Nếu không có modifier thì gửi type_text
    ...
    def mouse_move_norm(self, nx, ny):
        """Di chuyển chuột bằng tọa độ chuẩn hóa (0..1)."""
